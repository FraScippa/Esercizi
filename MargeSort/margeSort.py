...
def mergeSort(list_input: list[int]) -> list[int]:
    ...
    if len(list_input) == 1:
        ...
        return list_input #ritorno questa perchè mi servirà per il margesort
    
    mid_point: int = len(list_input) // 2 #usa divisione intere (nel caso in cui la lista è dispari)
    
    left_list: list[int] = mergeSort(list_input = list_input[:mid_point]) #la parte finale sempre esclusa
    right_list: list[int] = mergeSort(list_input = list_input[mid_point:])
    result: list[int] = merge(left_list, right_list)
    
    #return left_list + right_list non è corretto!! Perchè non è detto che siano ordinate.
    return result
...
def merge(list_1: list[int], list_2: list[int]) -> list[int]:
    ...
    for k in range(len(result)):
        ...
        if list_1[i] > list_2[j]:
            ...
            if j == len(list_2): #per evitare di andare out of bound
...
if __name__ == "__main__":
    ...
    cases = [10, 50, 100, 250, 500, 1000, 1500, 2000, 5000, 10000, 20000, 30000, 40000]

    for n in [10, 50, 100, 250, 500, 1000, 1500, 2000, 5000, 10000, 20000, 30000, 40000]:
    ...
    print(f"MERGE: {merge_sort_times}\nBUBBLE: {bubble_sort_times}") #nei primi numeri sembra che il bubbleSort sia più veloce
...
result: list[int] = mergeSort(list_input = list_input[::-1])